	...
	for(int i = 0; i < ylen; ++i)
	{
		...
		if(len > PIXEL_RUN)
		{
			...
			while(len > PIXEL_RUN)
			{
				...
				DrawHardwareSpanZBuffered(dstColPtr, dstZPtr, runLength, (ULONG)zStart, uStart, vStart, (LONG)iStart, dZz, dUUuu, dVVvv, dIi, lib);
		...
		if(len > 0)
		{
			...
			DrawHardwareSpanZBuffered(dstColPtr, dstZPtr, len, (ULONG)zStart, uStart, vStart, (LONG)iStart, dZz, dUUuu, dVVvv, dIi, lib);
		}
	}
}
